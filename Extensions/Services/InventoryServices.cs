...
namespace FieldMRIServices.Extensions.Services
{
    public class InventoryServices : IIventoryServices
    {
        ...
        public async Task<List<PhotoModel>> GetPhotoByInventoryIdAsync(int InventoryId)
        {
            var results = await _appDbContext.Photos.Where(_ => _.InventoryId == InventoryId).Include(_ => _.Inventory).ToListAsync();
            var list = results.Select(_mapper.Map<PhotoModel>);
            return list.ToList();
        }
        ...
        public async Task<List<PhotoModel>> GetPhotosAsync()
        {
            ...
            var photoModelList = photos.Select(_mapper.Map<PhotoModel>);
